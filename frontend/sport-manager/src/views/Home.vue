  ...
  <div class="home text-gray-900">
    ...
    <div class="relative z-0 w-full mb-6 group mt-5">
      ...
      <input
        ...
        v-model="teamName"
        id="teamName"
        class="block py-2.5 px-0 w-full text-sm text-black bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
        placeholder=""
        required
      />
      <label
  ...
  methods: {
    validateForm() {
      ...
      if (!this.teamName) {
        ...
        this.errors.teamName = 'Team Name must have a minimum of 3 characters.';
      }
    ...
    async sendMessage() {
      if (this.validateForm()) {
        try {
          const response = await axios.post('http://localhost/global-ventures/server/api/teams.php', {
